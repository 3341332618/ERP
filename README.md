...
该模块把 ERP 系统作为软件测试训练环境：管理员发布预置缺陷，学员在独立工作区完成测试并提交材料，管理员或终极管理员评分后生成排行榜和历史记录。这里的“发布缺陷”表示开启系统中的对应错误行为，不会把缺陷编号、摘要或任务清单展示给学员。

| 角色 | 能力 |
| --- | --- |
| 系统管理员 | 发布/关闭缺陷、管理学员、评阅缺陷报告、查看排行榜 |
| 终极管理员 | 管理学员、发布缺陷、评阅测试文件、查看操作轨迹、评分历史和排行榜 |
| 测试学员 | 使用独立 ERP 工作区自主发现缺陷、提交缺陷报告和测试文件、查看个人记录与排行榜；不显示已发布缺陷清单 |
