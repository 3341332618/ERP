...
- 测试学员拥有独立业务工作区，并自动配备采购专员、仓库专员、销售专员和结算主管子账号；不同学员的基础资料、单据、审核任务、库存和结算数据相互隔离。

## 测试竞赛扩展

该模块把 ERP 系统作为软件测试训练环境：管理员发布预置缺陷，每名学员拥有一套独立 ERP 工作区和岗位子账号，学员使用子账号完成采购、仓库、销售、结算等业务测试，再用学员主账号提交缺陷报告和测试材料。管理员或终极管理员评分后生成排行榜和历史记录。这里的“发布缺陷”表示开启系统中的对应错误行为，不会把缺陷编号、摘要或任务清单展示给学员。
...
系统按角色返回菜单。每名学员的 ERP 子账号只访问所属学员工作区，`student01_*` 与 `student02_*` 的业务数据互不共享。业务层还会校验管理员、终极管理员、仓库人员、工作区归属和单据所有者等权限。
