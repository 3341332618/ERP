...
> 测试竞赛采用“一个学员一套 ERP 工作区”模型：学员主账号负责提交缺陷报告和测试文件，系统会自动给每名学员生成采购、仓库、销售、结算 4 个 ERP 子账号，不同学员的数据互相隔离。
...
1. 先用学员 ERP 子账号测试业务，例如：
   - `student01_purchase_staff / 123456` 创建采购入库、采购退货。
   - `student01_warehouse_staff / 123456` 审核入库、出库、调拨。
   - `student01_sales_staff / 123456` 创建销售出库、销售退货。
   - `student01_settlement_manager / 123456` 查看收入和支出结算。
2. 学员在这套独立 ERP 工作区里自行探索异常行为。系统不会提示当前发布了哪些缺陷。
3. 发现异常后，登录 `student01 / 123456`。
4. 进入“我的缺陷报告”提交报告，报告标题、模块、复现步骤、预期结果、实际结果都由学员自己填写。
5. 上传测试文件。
6. 查看个人提交记录和排行榜。
