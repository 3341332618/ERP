...
6. 超级管理员发布一个缺陷，例如 `BUG-0004`。
7. 学员 `student01` 登录并提交 PDF 测试文件。
...
| TC-SA-007 | 学员文件提交 | 学员提交 `.pdf` 文件 | 返回提交记录，状态为 `PENDING` |
| TC-SA-008 | 文件评阅 | 超级管理员评分 88 分并通过 | 返回 `APPROVED`，分数为 88 |
| TC-SA-009 | 评分历史 | 查看 `/competition/history` | 存在本次评分轮次历史 |
| TC-SA-010 | 排行榜 | 查看 `/competition/rankings` | `student01` 总分包含文件评分 |
